...
export default function SignerPage() {
  ...
  function checkFreemiumLimit(): boolean {
    const count = parseInt(localStorage.getItem('ud_sig_count_v1') || '0', 10);
    if (count >= 3) {
      setPaywallMessage("Free Tier Exhausted. Mathematical tamper evidence and cryptographic validation requires the Founder Tier.");
      return false;
    }
    localStorage.setItem('ud_sig_count_v1', (count + 1).toString());
    return true;
  }
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px 80px' }}>
      ...
      <p style={{ marginTop: 48, textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--ud-muted)' }}>
        UD Signer runs locally in your browser. 3 free signatures per month.
      </p>

      {/* Global Paywall Modal */}
      {paywallMessage && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(10px)', padding: 20 }}>
          <div style={{ background: '#111', border: '1px solid rgba(212,175,55,0.5)', borderRadius: 16, width: '100%', maxWidth: 420, padding: 32, textAlign: 'center', position: 'relative', overflow: 'hidden', boxShadow: '0 0 50px rgba(212,175,55,0.1)' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: 4, background: 'linear-gradient(to right, transparent, #D4AF37, transparent)' }}></div>
            <h3 style={{ fontSize: 24, fontWeight: 700, color: '#fff', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.1em', fontFamily: 'var(--font-display)' }}>Upgrade Required</h3>
            <p style={{ color: '#aaa', marginBottom: 32, fontSize: 15, lineHeight: 1.6, fontFamily: 'var(--font-body)' }}>{paywallMessage}</p>
            <a 
              href="https://hive.baby"
              target="_blank"
              style={{ display: 'block', width: '100%', padding: '16px 0', background: '#D4AF37', color: '#000', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', borderRadius: 8, textDecoration: 'none', marginBottom: 16, boxShadow: '0 0 15px rgba(212,175,55,0.3)', fontFamily: 'var(--font-mono)', fontSize: 14 }}
            >
              View Founder Plan
            </a>
            <button 
              onClick={() => setPaywallMessage("")}
              style={{ background: 'none', border: 'none', color: '#666', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-mono)' }}
            >
              Cancel
            </button>
          </div>
