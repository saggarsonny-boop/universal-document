    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <span style={{ fontSize: 13, fontWeight: 600, padding: '3px 10px', borderRadius: 99, background: 'var(--ud-gold-3)', color: 'var(--ud-gold-text)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Pro · AI</span>
      ...
      <div style={{ marginBottom: 24 }}>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Media duration (optional)</label>
      ...
      {processing && <div style={{ marginBottom: 16 }}><div style={{ height: 4, background: 'var(--ud-border)', borderRadius: 99, overflow: 'hidden' }}><div style={{ height: '100%', width: '60%', background: 'var(--ud-teal)', borderRadius: 99, animation: 'ud-bounce 1.4s ease-in-out infinite' }} /></div><div style={{ fontSize: 13, color: 'var(--ud-muted)', fontFamily: 'var(--font-body)', marginTop: 8 }}>Aligning text with media timeline…</div></div>}
      ...
      {sync && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)', marginBottom: 14 }}>{sync.sync_points.length} sync points · Est. duration: {sync.total_duration_estimate}</div>
          <div style={{ border: '1px solid var(--ud-border)', borderRadius: 'var(--ud-radius-lg)', overflow: 'hidden', marginBottom: 16 }}>
            {sync.sync_points.map((pt, i) => (
              <div key={i} style={{ display: 'flex', gap: 16, padding: '10px 16px', borderBottom: i < sync.sync_points.length - 1 ? '1px solid var(--ud-border)' : 'none', background: i % 2 === 0 ? '#fff' : 'var(--ud-paper-2)' }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: 'var(--ud-teal)', minWidth: 48, fontWeight: 600 }}>{pt.timestamp}</span>
