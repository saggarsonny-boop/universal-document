    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '48px 24px' }}>
      ...
      {entries.map((entry, idx) => (
        <div key={entry.id} style={{ marginBottom: 16, padding: '16px 18px', background: 'var(--ud-paper-2)', border: '1px solid var(--ud-border)', borderRadius: 'var(--ud-radius-lg)' }}>
          <div style={{ fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12, display: 'flex', justifyContent: 'space-between' }}>
            <span>Entry {idx + 1}</span>
            {entries.length > 1 && (
              <button onClick={() => removeEntry(entry.id)} style={{ background: 'none', border: 'none', color: 'var(--ud-danger)', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)', padding: 0 }}>Remove</button>
            )}
          </div>
      ...
      {result && (
        <div style={{ padding: '14px 18px', background: 'var(--ud-teal-2)', border: '1px solid var(--ud-teal)', borderRadius: 'var(--ud-radius-lg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)' }}>{entries.length} {entries.length === 1 ? 'entry' : 'entries'} · {caseName.slice(0, 40)}</div>
          </div>
