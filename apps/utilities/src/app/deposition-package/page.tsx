...
export default function DepositionPackage() {
  ...
  const lbl = { display: 'block' as const, fontSize: 13, fontWeight: 600 as const, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)', textTransform: 'uppercase' as const, letterSpacing: '0.08em', marginBottom: 8 }
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Transcript <span style={{ color: 'var(--ud-danger)' }}>*</span></div>
          <div style={dropStyle(draggingT)} onDragOver={e => { e.preventDefault(); setDraggingT(true) }} onDragLeave={() => setDraggingT(false)} onDrop={onDropT} onClick={() => tRef.current?.click()}>
            ...
            <div style={{ fontSize: 13, color: transcript ? 'var(--ud-ink)' : 'var(--ud-muted)', fontFamily: 'var(--font-body)' }}>{transcript ? `📋 ${transcript.name}` : 'Drop transcript here'}</div>
          </div>
        </div>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Exhibits</div>
          <div style={dropStyle(draggingE)} onDragOver={e => { e.preventDefault(); setDraggingE(true) }} onDragLeave={() => setDraggingE(false)} onDrop={onDropE} onClick={() => eRef.current?.click()}>
            ...
            <div style={{ fontSize: 13, color: 'var(--ud-muted)', fontFamily: 'var(--font-body)' }}>{exhibits.length ? `${exhibits.length} exhibit${exhibits.length > 1 ? 's' : ''} added` : 'Drop exhibits (optional)'}</div>
          </div>
        </div>
      </div>
      ...
      {result && (
        <div style={{ padding: '14px 18px', background: 'var(--ud-teal-2)', border: '1px solid var(--ud-teal)', borderRadius: 'var(--ud-radius-lg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginBottom: 20 }}>
          <div><div style={{ fontSize: 14, fontWeight: 600, color: 'var(--ud-teal)', fontFamily: 'var(--font-body)' }}>Package ready · Output: .udz bundle</div><div style={{ fontSize: 13, fontFamily: 'var(--font-mono)', color: 'var(--ud-muted)' }}>1 transcript + {exhibits.length} exhibit{exhibits.length !== 1 ? 's' : ''}</div></div>
